...
export default function MisPrestamosPage() {
  ...
  const [confirmModal, setConfirmModal] = useState({ show: false, prestamo: null });
  ...
  const handleCancelar = (prestamo) => {
    setConfirmModal({ show: true, prestamo });
  };

  const confirmarCancelacion = async () => {
    const prestamo = confirmModal.prestamo;
    if (!prestamo) return;

    setConfirmModal({ show: false, prestamo: null });
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        onClose={() => setConfirmModal({ show: false, prestamo: null })}
        onConfirm={confirmarCancelacion}
        title="Cancelar Préstamo"
        message={`¿Estás seguro de que deseas cancelar este préstamo?\n\nLibro: ${confirmModal.prestamo?.libro?.titulo || 'N/A'}\n\nEsta acción no se puede deshacer.`}
        type="confirm"
